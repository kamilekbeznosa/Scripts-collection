...
def audit(
    target: str = typer.Argument(
        ..., help="Nazwa użytkownika lub organizacji na GitHubie (np. microsoft)"
    ),
    is_org: bool = typer.Option(
        False, "--org", help="Oznacz, jeśli cel to Organizacja, a nie pojedynczy użytkownik"
    ),
    ...
    if not token:
        typer.secho(
            "Błąd: Brak zmiennej środowiskowej GITHUB_TOKEN. Nigdy nie podawaj tokena jako flagi w CLI!",
            fg=typer.colors.RED,
        )
    ...
    if json_output:
        typer.echo(json.dumps({"target": target, "has_errors": has_errors, "issues": results}))
